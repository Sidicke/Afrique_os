  ...
  return (
    <div className="flex flex-col gap-6">
      ...
      <div className="grid grid-cols-2 gap-3 lg:grid-cols-4">
        ...
        <MiniStat icon="wallet" label="CA clients" value={formatCurrency(stats.totalSpent)} tone="green" />
        <MiniStat icon="basket" label="Panier moyen" value={formatCurrency(stats.avgBasket)} tone="ivory" />
      </div>
                ...
                {filtered.map((customer) => (
                  <tr key={customer.id} className="group transition-colors hover:bg-ink-50/70">
                    ...
                    <td className="px-2 py-3.5 text-right font-mono font-bold text-gold-strong">
                      {formatCurrency(customer.totalSpentFcfa)}
                    </td>
                    <td className="px-2 py-3.5">
